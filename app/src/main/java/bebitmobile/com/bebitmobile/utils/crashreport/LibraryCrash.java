...
public class LibraryCrash {
    ...
    private static final String EXTRA_IMAGE_DRAWABLE_ID = "";

    //General constants
    private final static String TAG = "LibraryCrash";
    ...
    private static int defaultErrorActivityDrawableId = R.drawable.image_crash;
    ...
    /**
     * Installs LibraryCrash on the application using the default error activity.
     *
        ...
        try {
            if (context == null) {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    Log.w(TAG, "LibraryCrash will be installed, but may not be reliable in API lower than 14");
                }
                ...
                if (oldHandler != null && oldHandler.getClass().getName().startsWith(CAOC_HANDLER_PACKAGE_NAME)) {
                    Log.e(TAG, "You have already installed LibraryCrash, doing nothing!");
                } else {
                    if (oldHandler != null && !oldHandler.getClass().getName().startsWith(DEFAULT_HANDLER_PACKAGE_NAME)) {
                        Log.e(TAG, "IMPORTANT WARNING! You already have an UncaughtExceptionHandler, are you sure this is correct? If you use ACRA, Crashlytics or similar libraries, you must initialize them AFTER LibraryCrash! Installing anyway, but your original handler will not be called.");
                    }
                    ...
                    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
                        ...
                        public void uncaughtException(Thread thread, final Throwable throwable) {
                            Log.e(TAG, "App has crashed, executing LibraryCrash's UncaughtExceptionHandler", throwable);
                    ...
                    Log.i(TAG, "LibraryCrash has been installed.");
                }
            }
        } catch (Throwable t) {
            Log.e(TAG, "An unknown error occurred while installing LibraryCrash, it may not have been properly initialized. Please report this as a bug if needed.", t);
        }
    }
    ...
    public static boolean isShowErrorDetailsFromIntent(Intent intent) {
        return intent.getBooleanExtra(LibraryCrash.EXTRA_SHOW_ERROR_DETAILS, true);
    }
    ...
    public static int getDefaultErrorActivityDrawableIdFromIntent(Intent intent) {
        return intent.getIntExtra(LibraryCrash.EXTRA_IMAGE_DRAWABLE_ID, R.drawable.image_crash);
    }
    ...
    public static String getStackTraceFromIntent(Intent intent) {
        return intent.getStringExtra(LibraryCrash.EXTRA_STACK_TRACE);
    }
    ...
    public static Class<? extends Activity> getRestartActivityClassFromIntent(Intent intent) {
        Serializable serializedClass = intent.getSerializableExtra(LibraryCrash.EXTRA_RESTART_ACTIVITY_CLASS);
    ...
    public static void setLaunchErrorActivityWhenInBackground(boolean launchErrorActivityWhenInBackground) {
        LibraryCrash.launchErrorActivityWhenInBackground = launchErrorActivityWhenInBackground;
    }
    ...
    public static void setShowErrorDetails(boolean showErrorDetails) {
        LibraryCrash.showErrorDetails = showErrorDetails;
    }
    ...
    public static void setDefaultErrorActivityDrawable(int defaultErrorActivityDrawableId) {
        LibraryCrash.defaultErrorActivityDrawableId = defaultErrorActivityDrawableId;
    }
    ...
    public static void setEnableAppRestart(boolean enableAppRestart) {
        LibraryCrash.enableAppRestart = enableAppRestart;
    }
    ...
    public static void setErrorActivityClass(Class<? extends Activity> errorActivityClass) {
        LibraryCrash.errorActivityClass = errorActivityClass;
    }
    ...
    public static void setRestartActivityClass(Class<? extends Activity> restartActivityClass) {
        LibraryCrash.restartActivityClass = restartActivityClass;
    }
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.mentimun.pasarrakyat.crashReport.RESTART" />,
    ...
    private static Class<? extends Activity> guessRestartActivityClass(Context context) {
        ...
        resolvedActivityClass = LibraryCrash.getRestartActivityClassWithIntentFilter(context);
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.mentimun.pasarrakyat.crashReport.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.mentimun.pasarrakyat.crashReport.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(Context context) {
        ...
        //If action is defined, use that
        resolvedActivityClass = LibraryCrash.getErrorActivityClassWithIntentFilter(context);

        //Else, get the default launcher activity
        if (resolvedActivityClass == null) {
            resolvedActivityClass = CustomCrash.class;
        }
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.mentimun.pasarrakyat.crashReport.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
